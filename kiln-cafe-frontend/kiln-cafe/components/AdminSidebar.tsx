...
export default function AdminSidebar() {
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-forest text-white border-b border-white/10 h-16 flex items-center justify-between px-4">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 bottom-0 w-64 bg-forest text-white z-40 transform transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-heading font-semibold text-sm truncate">{user?.name}</p>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <Link
            href="/"
            onClick={() => setIsMobileMenuOpen(false)}
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-cream/80 hover:bg-white/10 hover:text-white transition-all"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className="font-heading font-medium">Back to Website</span>
          </Link>
          <button
            onClick={() => {
